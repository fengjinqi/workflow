                ...
                <div class="flex" style="height: 300px;">
                    <div class="left">
                        <template v-if="data.orderStatus=='待确认'">
                            <div><el-button type="primary" class="cancel" style="color: #26B7BC;border-color: #26B7BC;" @click="remove(data.tradeId)">取消订单</el-button></div>
                            <div><el-button type="primary" style="background: #26B7BC;border-color: #26B7BC;" class="ok" @click="sub(data.tradeId)">确认订单</el-button></div>
                        </template>
                        <template v-else-if="data.orderStatus=='审核中'">
                            <div><el-button type="primary" class="ok"  style="background: #26B7BC;border-color: #26B7BC;" @click="remove(data.tradeId)">取消订单</el-button></div>
                        </template>
                        <template v-else-if="data.orderStatus=='待发货'">
                            <div><el-button type="primary" class="ok" style="background: #26B7BC;border-color: #26B7BC;">订单退款</el-button></div>
                        </template>
                        <template v-else-if="data.orderStatus=='待收货'">
                            <div><el-button type="primary" class="cancel" style="color: #26B7BC;border-color: #26B7BC;" >查看物流</el-button></div>
                            <div><el-button type="primary" class="ok"  style="background: #26B7BC;border-color: #26B7BC;">确认签收</el-button></div>
                        </template>
                        <template v-else-if="data.orderStatus=='已完成'">
                            <div><el-button type="primary" class="ok"  style="background: #26B7BC;border-color: #26B7BC;">申请售后</el-button></div>

                        </template>
                        <template v-else-if="data.orderStatus=='已取消'">
                            <div><el-button type="primary"  disabled="" style="background: #26B7BC;border-color: #26B7BC;">订单已取消</el-button></div>
                        </template>
                        ...
                        <div class="item">
                            ...
                            <div class="cont">
                                <el-steps :active="data.orderStatus=='待确认'?1:data.orderStatus=='审核中'?2:data.orderStatus=='待发货'?3:data.orderStatus=='待收货'?4:data.orderStatus=='已完成'?5:data.orderStatus=='使用中'?5:''">
                                    <el-step title="待确认" ><slot ><img :src="dai" alt=""></slot></el-step>
